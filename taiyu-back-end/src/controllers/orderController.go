...
func CreateOrder(c *fiber.Ctx) error{
	...
	stripe.Key = "sk_test_51LeEEgHNGVAdcCcp6IVVpqd0kbl1VBn3G0r025l7X44rZ80Rtq6dTktkDYEfGRgeqclmYuP1p3aehb7SoYKl0b5h00fNoY6uvQ"
	params := stripe.CheckoutSessionParams{
		SuccessURL: stripe.String("http://localhost:5000/suscess?source={CHECKOUT_SESSION_IO}"),
...
func CompleteOrder(c *fiber.Ctx)error{
	...
	order := models.Order{}
	database.DB.Preload("OrdeItems").First(&order, models.Order{
	...
	go func (order models.Order)  {
		...
		ambassadorMessage := []byte(fmt.Sprintf("You earn $%f from the link #%s", ambassadorRevenue, order.Code))
		smtp.SendMail("0.0.0.0:1025", nil, "no-reply@email.com",[]string{order.AmbassadorEmail}, ambassadorMessage)
		adminMessage := []byte(fmt.Sprintf("Order #%d with a total of $%f been complete", order.Id, adminRevenue))
